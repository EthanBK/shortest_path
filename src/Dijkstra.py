...
boundary = np.transpose(np.array([[math.ceil(inFile.header.min[0]),
                                   math.ceil(inFile.header.min[1]),
                                   math.ceil(inFile.header.min[2])],
                                  [math.floor(inFile.header.max[0]),
                                   math.floor(inFile.header.max[1]),
                                   math.floor(inFile.header.max[2])]]))
...
nodes = [None for i in range((boundary[0, 1] - boundary[0, 0]) *
                             (boundary[1, 1] - boundary[1, 0]) *
                             (boundary[2, 1] - boundary[2, 0]) / pow(c_dis, 3))]
print("nodes.length: ", nodes)

node_count = 0

for i in range(boundary[0, 0], boundary[0, 1], c_dis):
    for j in range(boundary[1, 0], boundary[1, 1], c_dis):
        for k in range(boundary[2, 0], boundary[2, 1], c_dis):
            near_point = point_tree.query_ball_point([i, j, k], c_dis)
            # if obstacle within c_dis, do not create node
            if len(near_point) != 0:
                continue

            newNode_index = (i - boundary[0, 0]) / c_dis
            newNode_index += (boundary[0, 1] - boundary[0, 0]) * (j - boundary[1, 0]) / c_dis
            newNode_index += (boundary[1, 1] - boundary[1, 0]) * (boundary[0, 1] - boundary[0, 0]) * \
                             (k - boundary[2, 0]) / c_dis
                ...
                for q in range(-1, 2):
                    for r in range(-1, 2):
                        if p == 0 and q == 0 and r == 0:
                            continue
                        nei_index = newNode_index + p + \
                                    q * (boundary[0, 1] - boundary[0, 0]) + \
                                    r * (boundary[1, 1] - boundary[1, 0]) * (boundary[0, 1] - boundary[0, 0])

                        if 0 <= nei_index[0] < len(nodes) and \
                                0 <= nei_index[1] < len(nodes[0]) and \
                                0 <= nei_index[2] < len(nodes[0][0]) and \
                                nodes[nei_index[0]][nei_index[1]][nei_index[2]] is not None:
            ...
            nodes[newNode_index[0]][newNode_index[1]][newNode_index[2]] = newNode
...
while sourceNode is None:
    x = random.random() * len(nodes)
    y = random.random() * len(nodes[0])
    z = random.random() * len(nodes[0][0])
    sourceNode = nodes[x][y][z]
