...
class KdTree:
    ...
    def insert_node(self, id, node, parent, point, compare):
        ...
        if node is None:
            if self.size == 0:
                self.size += 1
                cube = CubeHV(float('-inf'), float('inf'),
                              float('-inf'), float('inf'),
                              float('-inf'), float('inf'))
                # print(id, point.to_string())
                # print(cube.to_string())
                return Node(id, point, cube, 0)
            ...
            if compare < 0:
                if parent.direction == 0:
                    cube = CubeHV(parent.cube.xmin, parent.cube.xmax,
                                  parent.cube.ymin, parent.cube.ymax,
                                  parent.cube.zmin, parent.point.z)
                elif parent.direction == 1:
                    cube = CubeHV(parent.cube.xmin, parent.cube.xmax,
                                  parent.cube.ymin, parent.point.y,
                                  parent.cube.zmin, parent.cube.zmax)
                elif parent.direction == 2:
                    cube = CubeHV(parent.cube.xmin, parent.point.x,
                                  parent.cube.ymin, parent.cube.ymax,
                                  parent.cube.zmin, parent.cube.zmax)
                else:
                ...
                if parent.direction == 0:
                    cube = CubeHV(parent.cube.xmin, parent.cube.xmax,
                                  parent.cube.ymin, parent.cube.ymax,
                                  parent.point.z, parent.cube.zmax)
                elif parent.direction == 1:
                    cube = CubeHV(parent.cube.xmin, parent.cube.xmax,
                                  parent.point.y, parent.cube.ymax,
                                  parent.cube.zmin, parent.cube.zmax)
                elif parent.direction == 2:
                    cube = CubeHV(parent.point.x, parent.cube.xmax,
                                  parent.cube.ymin, parent.cube.ymax,
                                  parent.cube.zmin, parent.cube.zmax)
                else:
                    raise("Direction Error!")
            self.size += 1
            return Node(id, point, cube, (parent.direction + 1) % 3)
